# FUTURE_UX_01
"Latte Love" is a cozy Hyderabad coffee spot designed for professionals, students &amp; friends. The website features a warm espresso-brown aesthetic, a 3-page layout with menu, table reservations &amp; WhatsApp booking — all built to turn first-time visitors into loyal regulars. ☕
# ☕ Latte Love Coffee Café -

## Overview

**Latte Love Coffee Café** is a modern coffee shop mobile application designed to provide users with a seamless and enjoyable coffee ordering experience. This project was completed as part of my **UI/UX Design Internship**, focusing on creating an intuitive, visually appealing, and user-centered interface.

The design emphasizes simplicity, accessibility, and a warm café-inspired aesthetic while enabling users to browse the menu, customize drinks, place orders, and make secure payments effortlessly.

---

## Problem Statement

Many coffee ordering applications overwhelm users with cluttered interfaces and complicated ordering processes. The objective of this project was to design a clean, intuitive, and engaging mobile experience that allows customers to discover products, customize orders, and complete purchases quickly.

---

## Objectives

* Design a visually appealing coffee café application.
* Create a simple and intuitive user experience.
* Improve navigation and accessibility.
* Streamline the ordering and checkout process.
* Build a consistent design system with reusable components.

---

## Design Process

The project followed a user-centered design approach:

1. User Research
2. Competitor Analysis
3. User Flow Creation
4. Low-Fidelity Wireframes
5. High-Fidelity UI Design
6. Interactive Prototype
7. Design Evaluation & Iteration

---

## Key Features

* Welcome & Onboarding Screens
* User Login & Registration
* Home Screen with Featured Coffee
* Search Functionality
* Coffee Categories
* Product Details
* Drink Customization
* Shopping Cart
* Secure Checkout
* Order Confirmation
* User Profile

---

## Design System

### Color Palette

| Color      | Purpose             |
| ---------- | ------------------- |
| Cream      | Background          |
| Brown      | Primary Brand Color |
| Teal       | Accent Color        |
| White      | Cards & Content     |
| Dark Brown | Headings & Text     |

### Typography

* Clean, modern sans-serif fonts
* Clear visual hierarchy
* Readable spacing and sizing

### Components

* Buttons
* Cards
* Navigation Bar
* Search Bar
* Coffee Product Cards
* Quantity Selector
* Input Fields
* Icons

---

## Tools Used

* **Figma** – UI Design & Prototyping
* **FigJam** – User Flows & Brainstorming

---

## Coffee Categories Included 
Expresso
Cappuccino
Latte
Mocha
Americano
Cold Brew

## Screens Included

* Splash Screen
  <img width="392" height="862" alt="coffe cafe" src="https://github.com/user-attachments/assets/809f4ba2-abcc-4e15-b2f1-c64ea3b2cf5d" />
* Login
* <img width="392" height="862" alt="iPhone 13   14 - 1" src="https://github.com/user-attachments/assets/479c746b-b14a-4d15-b96e-c293a3729f57" />
* Sign Up
* <img width="393" height="862" alt="iPhone 13   14 - 4" src="https://github.com/user-attachments/assets/a9c9140c-53c0-4def-829f-0a7b400345ec" />
* Home
* <img width="392" height="862" alt="iPhone 13   14 - 3" src="https://github.com/user-attachments/assets/18eb01bc-02ba-4eb5-8e32-2da8351fcd37" />
* Product Details
* <img width="392" height="862" alt="iPhone 13   14 - 5" src="https://github.com/user-attachments/assets/1c4a6467-a969-4cc0-9b2e-a1991c27123f" />
* Cart
* <img width="392" height="862" alt="iPhone 13   14 - 6" src="https://github.com/user-attachments/assets/b13e5578-3c80-4493-bac5-4bf02ee90184" />
* Payment
* <img width="392" height="861" alt="iPhone 13   14 - 7" src="https://github.com/user-attachments/assets/adb36e66-aa9f-47a8-975b-3340fe63aa3b" />
* Order Success
* <img width="392" height="862" alt="iPhone 13   14 - 8" src="https://github.com/user-attachments/assets/7e80a9f0-394f-4786-80f2-dc2de18de448" />

---

## Learning Outcomes

Through this internship project, I gained practical experience in:

* UI/UX Design Principles
* User-Centered Design
* Wireframing
* High-Fidelity Interface Design
* Design Systems
* Color Theory
* Typography
* Auto Layout in Figma
* Interactive Prototyping
* Component-Based Design
* Responsive Design Concepts

---

## Project Highlights

* Modern café-inspired interface
* Minimal and clean design
* Consistent visual language
* User-friendly navigation
* Interactive prototype
* Scalable component library

---
## Project Links
## Figma Design File 
https://www.figma.com/design/G6bZdbAzC51OHj5axN1f1j/latte-love?node-id=0-1&t=gCc7QlOq1iZQhNNs-1
## Interactive Prototype
https://www.figma.com/proto/G6bZdbAzC51OHj5axN1f1j/latte-love?node-id=0-1&t=gCc7QlOq1iZQhNNs-1

## Designed By

**Meghana Reddy**

UI/UX Internship Project-
FUTURE_UX_01
Github : https://github.com/cmeghanareddy04
LinkedIn: https://linkedin.com/in/Meghanaachegireddy
